...
int main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "2hvs")) != EOF)
	{
		switch(ch) {
		case '2':
			use_llc_type = 2;
	...
	if(use_llc_type == 2)
	{
		struct sockaddr_llc from_ac;
		int from_ac_len = sizeof(struct sockaddr_llc);
		...
		s2 = accept(s, (struct sockaddr *)&from_ac, &from_ac_len);
		if(s2 < 0)
		{
			perror("llcpingd: accept");
			exit (2);
		}
	}
	else
	...
	for(;;)
	{
		...
		fromlen = sizeof(from);
		if((cc = recvfrom(s2, (char *)packet, packlen, 0,
		    (struct sockaddr *)&from, &fromlen)) < 0)
		{
			if(errno == EINTR)
				continue;
			perror("llcpingd: recvfrom");
			continue;
		}

		printf("RX: %02X:%02X:%02X:%02X:%02X:%02X @ 0x%02X"
		...
		(void) sendto(s2, packet, cc, 0, (struct sockaddr *)&loop, sizeof(loop));
		free(packet);
	}
